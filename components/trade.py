...
class TradeModel(BaseModel):
  def __init__(self, args, tokenizer, ontology):
    super(TradeModel, self).__init__(args, None, None, ontology)
    self.name = "TRADE"
    self.args = args
    # lang, SLOTS_LIST, gating_dict = tokenizer
    [self.lang, self.mem_lang], [self.slots, self.SLOTS_LIST, self.slot_temp, _], self.gating_dict = tokenizer
    ...
    if args.checkpoint:
      self.save_model_dir = args.checkpoint
    else:
      self.save_model_path = os.path.join(args.output_dir, args.model, args.task, args.dataset)

    self.encoder = EncoderRNN(self.lang.n_words, self.hidden_size, self.drop_rate)
    self.decoder = Generator(self.lang, self.encoder.embedding, self.lang.n_words,
                             self.hidden_size, self.drop_rate, self.slots, self.nb_gate)

    if self.save_model_path and os.path.exists(self.save_model_path):
      if USE_CUDA:
        trained_encoder = torch.load(os.path.join(self.save_model_path, 'enc.th'))
        trained_decoder = torch.load(os.path.join(self.save_model_path, 'dec.th'))
        print(f"MODEL {self.save_model_path} LOADED")
      else:
        trained_encoder = torch.load(os.path.join(self.save_model_path, 'enc.th'),lambda storage, loc: storage)
        trained_decoder = torch.load(os.path.join(self.save_model_path, 'dec.th'),lambda storage, loc: storage)
        print(f"MODEL {self.save_model_path} LOADED")
  ...
  def save_model(self, dec_type):          
    ...
    torch.save(self.encoder, os.path.join(self.save_model_path, 'enc.th'))
    torch.save(self.decoder, os.path.join(self.save_model_path, 'dec.th'))

  def reset(self):
    self.loss, self.print_every, self.loss_ptr, self.loss_gate, self.loss_class = 0, 1, 0, 0, 0

  def train_batch(self, data, clip=10, reset=0):
    if reset: self.reset()
    # Zero gradients of both optimizers
    self.optimizer.zero_grad()
    
    # Encode and Decode
    use_teacher_forcing = random.random() < self.args.teacher_force
    all_point_outputs, gates, words_point_out, words_class_out = self.encode_and_decode(data, use_teacher_forcing, self.SLOTS_LIST)
  ...
  def encode_and_decode(self, data, use_teacher_forcing, slot_temp):
    ...
    if self.decoder.training:
      ...
      bi_mask = np.random.binomial([np.ones((story_size[0],story_size[1]))], 1-self.drop_rate)[0]
  ...
  def evaluate(self, dev, matric_best, early_stop=None):
    ...
    if not self.args.do_train:
      json.dump(all_prediction, open(os.path.join(self.save_model_path, f"all_prediction_{self.name}.json"), 'w'), indent=2)
...
class EncoderRNN(nn.Module):
  def __init__(self, vocab_size, hidden_size, dropout, n_layers=1):
    ...
    self.embedding = nn.Embedding(vocab_size, hidden_size, padding_idx=self.PAD_token)
