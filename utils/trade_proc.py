...
def SaveData(data, data_dir):
    with open(os.path.join(data_dir, "testListFile.txt")) as df:
        testListFile=df.read().lower().split("\n")
        testListFile.remove('')
    with open(os.path.join(data_dir, "valListFile.txt")) as df:
        valListFile=df.read().lower().split("\n")
        valListFile.remove('')
    ...
    for dialogue_name in data:
        ...
        if dial:
            ...
            if dialogue_name.lower() in testListFile:
                test_dials.append(dialogue)
            elif dialogue_name.lower() in valListFile:
    ...
    # save all dialogues
    with open(os.path.join(data_dir, "dev_dials_trade.json"), 'w') as f:
        json.dump(dev_dials, f, indent=2)

    with open(os.path.join(data_dir, "test_dials_trade.json"), 'w') as f:
        json.dump(test_dials, f, indent=2)

    with open(os.path.join(data_dir, "train_dials_trade.json"), 'w') as f:
        json.dump(train_dials, f, indent=2)

def trade_process(args):
    data_dir = os.path.join(args.input_dir, "multiwoz_dst/MULTIWOZ2.1/")
    if os.path.exists(os.path.join(data_dir, "train_dials.json")) and \
       os.path.exists(os.path.join(data_dir, "dev_dials.json")) and \
       os.path.exists(os.path.join(data_dir, "test_dials.json")):
        pass
