  ...
  if args.auto_carry:
    for convo_id, convo_turns in progress_bar(preds.items(), total=len(preds)):
      ...
      for turn_count, turn_tuple in convo_turns.items():
        ...
        if pred == 'none':
          previous_value = preds_so_far[slot]
          if previous_value != 'none':
            pred = previous_value   # then carry over the old value

        convo_turns[turn_count] = (pred, slot, value)
        preds_so_far[slot] = pred  # store it for the next round
